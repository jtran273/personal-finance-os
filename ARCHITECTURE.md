...
| `/dashboard` | Simplified balance dashboard with Net worth, Income / liquid assets, and Cash flow views, sync freshness, selected-period transaction activity, category spending, and mobile summary | Accounts, snapshots, transactions |
...
`src/lib/finance/balances.ts` derives account totals, sync freshness, and balance trends from accounts, balance snapshots, and transaction history. The dashboard exposes three top-level views in this order: Cash flow, Inflows / liquid assets, and Net worth. Internally those map to `cashMinusLiabilities`, `cash`, and `netWorth`; the dashboard defaults to Cash flow because that is the day-to-day operating view. Desktop layouts render the interactive balance chart; mobile layouts use a simplified balance summary with the same range controls and transaction link so phone views avoid horizontal chart overflow. Selecting a point in the desktop cash-flow trend surfaces the related non-transfer transactions and links back to the transaction filters. The Inflows / liquid assets breakdown keeps its own range state and defaults to all reportable positive non-transfer inflows across accounts so the drilldown opens income-only transactions instead of inheriting the Cash flow chart's shorter range. Exact refund/reversal pairs are excluded from that reportable inflow set instead of being counted as income.

`src/lib/finance/liabilities.ts` powers the dashboard credit-card action panel from active credit accounts, cash balances, credit limits, Plaid liabilities due dates/minimums when available, and likely payment transactions. The panel ranks cards by overdue/due-soon status, cash-covered minimums, utilization pressure, and balance size so the first row is the most actionable payment candidate.

`src/lib/finance/spending.ts` powers category spending breakdowns, spending confidence, reimbursement-aware totals, and cleanup quality flags. The dashboard category panel lives under Cash flow and can show cumulative category trends for the selected range or month-by-month category rows for the last six months. The separate `budget-guardrails.ts` helper remains available for deterministic guardrail summaries, but it is not the primary dashboard surface today.
