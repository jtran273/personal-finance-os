...
`src/lib/finance/balances.ts` derives account totals, sync freshness, and balance trends from accounts, balance snapshots, and transaction history. The dashboard exposes three top-level views: Net worth, Income / liquid assets, and Cash flow. Internally those map to `netWorth`, `cash`, and `cashMinusLiabilities`; standalone debt is folded into net-worth composition and cash-flow math instead of being a separate dashboard tab. Desktop layouts render the interactive balance chart; mobile layouts use a simplified balance summary with the same range controls and transaction link so phone views avoid horizontal chart overflow. Selecting a point in the desktop cash-flow trend surfaces the related non-transfer transactions and links back to the transaction filters.

`src/lib/finance/liabilities.ts` remains available for credit-card due-date summaries from active credit accounts, cash balances, credit limits, and likely payment transactions. The simplified dashboard no longer renders that as a standalone panel; debt is visible through net-worth composition and cash-flow math instead.
