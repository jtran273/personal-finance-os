...
Core reimbursement-link helpers can attach a received positive inflow to an existing reimbursement record, preserve partial outstanding balances, mark the received enriched row as reimbursable so it does not inflate income reports, and write audit events for link/unlink decisions without changing raw provider rows. A positive inflow can also be marked as unmatched reimbursement income when no expected reimbursement record is available yet; this uses the enriched transaction intent, is queryable through reimbursable transaction state, and can be cleared back to personal or business income with an audit event. A future migration can add a dedicated unmatched-inflow table if Tally needs richer lifecycle fields beyond the current enriched-row state. Tally can also rank likely peer-to-peer reimbursement inflows against outstanding shared or reimbursable expenses, but those deterministic suggestions still require explicit user confirmation before any write.
