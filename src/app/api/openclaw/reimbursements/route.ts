...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return jsonNoStore(buildOpenClawReimbursementsResponse(transactions, { limit }));
  } catch (error) {
    ...
    logSafeError("openclaw_reimbursements_failed", error);
    return jsonNoStore({ error: "Unable to load OpenClaw reimbursements." }, { status: 500 });
  }
}
