...
import { listReviewItems } from "@/lib/db";
import {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logSafeError("openclaw_review_items_failed", error);
    return jsonNoStore({ error: "Unable to load OpenClaw review items." }, { status: 500 });
  }
}
