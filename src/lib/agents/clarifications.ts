...
export function normalizeReimbursementClarificationAnswer(answer: string): NormalizedClarificationAnswer {
  ...
  if (splitMatch?.[1]) {
    const counterparties = splitCounterpartyText(splitMatch[1])
  ...
  const counterpartyMatch = rawAnswer.match(/^([A-Za-z][A-Za-z.'-]*)(?:\s+(?:dinner|lunch|brunch|coffee|drinks|rent|utilities|hotel|travel|trip|tickets|gift|reimbursement|reimbursed|paid|payback))?$/i);
