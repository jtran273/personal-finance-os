...
test("suggestReimbursementMatches ranks by outstanding reimbursement amount instead of gross expense", () => {
  const suggestions = suggestReimbursementMatches([expense()], [
    inflow({ amount: 121.35, id: "gross-expense-sized", merchant: "Venmo - Chris L." }),
    inflow({ amount: 75, id: "expected-sized", merchant: "Venmo - Chris L." })
  ]);

  assert.deepEqual(suggestions[0].inflowIds, ["expected-sized"]);
  assert.equal(suggestions[0].confidence, "high");
});
